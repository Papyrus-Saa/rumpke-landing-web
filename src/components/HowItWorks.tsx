...
const HowItWorks = () => (
  <div className="dark:bg-dark-200 bg-light-100 w-full xl:w-[80%] 2xl:w-[70%] lg:p-6 mb-6 mx-auto shadow border dark:border-dark-100 rounded-lg border-light-200 transition-all duration-300">
    <h3 className={`${titleFonts.className} ml-2 mb-4 text-lg text-center`}>
      So einfach geht<span className="text-mint-600">&apos;</span>s
    </h3>
    <div className="relative bg-white dark:bg-dark-300 transition-all duration-300 p-4 flex flex-col lg:flex-row justify-center items-center gap-6">
      <div className="w-full lg:w-1/2 flex items-center">
        <ul className="space-y-4 mb-6 lg:mb-0 w-full">
          {steps.map((step, idx) => (
            <li
              key={idx}
              className={`flex items-center gap-4 rounded-xl px-4 w-full py-4 mx-auto bg-light-100 dark:bg-dark-200 transition-all duration-300 text-base sm:text-lg ${step.className}`}
            >
