  ...
  return (
    <div>
      ...
      >
        ...
        <p className={`${titleFonts.className} duration-500 text-xs mr-4 text-dark-300 dark:text-white`}>Schon teilgenommen</p>
        <span className={`${titleFonts.className} rounded-md px-2 text-white ml-auto bg-mint-600 dark:bg-mint-700`}>{total}</span>

        <div
          className={`absolute inset-0 flex items-center justify-center bg-white/10 dark:bg-dark-300/60 border border-white/30 dark:border-dark-100/30 rounded backdrop-blur-md z-10 transition-opacity duration-500 ease-in-out ${hover && total > 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        >
