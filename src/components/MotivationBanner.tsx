...
const MotivationBanner: React.FC = () => (
  <section className=" py-10  w-full">
    <h2 className="text-center text-2xl md:text-3xl font-bold text-mint-600  mb-6">
    ...
    <div className="duration-100 flex flex-col md:flex-row items-center justify-center gap-0 xl:w-[80%] 2xl:w-[70%] sm:w-[90%] mx-auto relative mb-6 ">
      {timeline.map((step) => (
        <div
          key={step.title}
          className="duration-100 flex flex-col items-center shadow-[var(--shadow-subtle-l)] dark:shadow-[var(--shadow-subtle-d)] border-mint-600  rounded-2xl text-center px-4 py-6 relative flex-1 m-2 hover:bg-mint-600/20"
        >
          <div className="duration-100 z-10 rounded-full p-4 shadow-[var(--shadow-subtle-l)] dark:shadow-[var(--shadow-subtle-d)] mb-4 border-4 border-mint-600 ">
            {step.icon}
          </div>
          <h2 className="duration-100 text-lg font-semibold text-mint-600 mb-2">{step.title}</h2>
