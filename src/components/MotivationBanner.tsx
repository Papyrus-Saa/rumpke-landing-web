...
const MotivationBanner: React.FC = () => (
  <section className="bg-white dark:bg-dark-300 py-10  w-full">
    ...
    <div className="flex flex-col md:flex-row items-center justify-center gap-0 xl:w-[80%] 2xl:w-[70%] sm:w-[90%] mx-auto relative mb-6 ">
      {timeline.map((step) => (
        <div
          key={step.title}
          className="flex flex-col items-center shadow-mint-600 shadow hover:shadow-xl transition-shadow border-mint-600 rounded-2xl text-center px-4 py-6 relative flex-1 dark:bg-dark-200 bg-light-100 m-2 hover:bg-mint-600/20"
        >
          <div className="z-10 bg-light-100 dark:bg-dark-100 rounded-full p-4 shadow-lg mb-4 border-4 border-mint-200  dark:border-mint-700">
            {step.icon}
          </div>
          <h3 className="text-lg font-semibold text-mint-700 dark:text-mint-400 mb-2">{step.title}</h3>
          <p className="text-gray-700 dark:text-gray-200 text-base ">{step.desc}</p>
        </div>
      ))}
    </div>
