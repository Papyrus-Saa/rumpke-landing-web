...
const timeline: TimelineStep[] = [
  {
    icon: <FaHome className="duration-500 text-mint-600 dark:text-mint-700 text-3xl" />,
    title: "Rumpke Immobilien",
    desc: "Die erfahrene Immobilienagentur aus der Region – zuverlässig, transparent und persönlich.",
  },
  {
    icon: <FaLightbulb className="duration-500 text-mint-600 dark:text-mint-700 text-3xl" />,
  ...
  {
    icon: <FaHandshake className="duration-500 text-mint-600 dark:text-mint-700 text-3xl" />,
  ...
  {
    icon: <FaGift className="duration-500  text-mint-600 dark:text-mint-700 text-3xl" />,
  ...
  <section className=" py-10  w-full">
    ...
    <div className="duration-500 flex flex-col md:flex-row items-center justify-center gap-0 xl:w-[80%] 2xl:w-[70%] sm:w-[90%] mx-auto relative mb-6 ">
      {timeline.map((step) => (
        <div
          key={step.title}
          className="duration-500 flex flex-col items-center shadow-[var(--shadow-subtle-l)] dark:shadow-[var(--shadow-subtle-d)] border-mint-600 dark:border-mint-700 rounded-2xl text-center px-4 py-6 relative flex-1 m-2 hover:bg-mint-600/20"
        >
          <div className="duration-500 z-10 rounded-full p-4 shadow-[var(--shadow-subtle-l)] dark:shadow-[var(--shadow-subtle-d)] mb-4 border-4 border-mint-600 dark:border-mint-700">
            {step.icon}
          </div>
          <h3 className="duration-500 text-lg font-semibold text-mint-600 dark:text-mint-700 mb-2">{step.title}</h3>
